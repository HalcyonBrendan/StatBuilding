...
class WinnerPlots():
	...
	def __init__(self,game_data):
		...
		for game in training_games:
			if count == 0:
				training_df = pd.DataFrame([[game["gameID"], game["isWinner"], 0, game["away"]["closeTo"], game["away"]["stats"][0], game["away"]["stats"][1]]], columns=["gameID","isHome","closeTo","coord1","coord2"])
				training_df = training_df.append(pd.DataFrame([[game["gameID"], game["isWinner"], 1, game["home"]["closeTo"], game["home"]["stats"][0], game["home"]["stats"][1]]], columns=["gameID","isHome","closeTo","coord1","coord2"]))
			else:
				training_df = training_df.append(pd.DataFrame([[game["gameID"], game["isWinner"], 0, game["away"]["closeTo"], game["away"]["stats"][0], game["away"]["stats"][1]]], columns=["gameID","isHome","closeTo","coord1","coord2"]))
				training_df = training_df.append(pd.DataFrame([[game["gameID"], game["isWinner"], 1, game["home"]["closeTo"], game["home"]["stats"][0], game["home"]["stats"][1]]], columns=["gameID","isHome","closeTo","coord1","coord2"]))
			count += 1
	...
	def build_plots(self,thresh):
		...
		self.plot_winners(rel_df,irrel_df,thresh)


	def plot_winners(self,rel_df,irrel_df,thresh):
		...
		plt.title(title_str)
		save_string = str(self.game_id) + "winner_view.png"
